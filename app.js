...
for (let gymId = 0; gymId < game.gyms.length; gymId++) {
    if (game.gyms[gymId].difficulty < 3) {
        game.gyms[gymId].completed = true
    }
}   
...
/*
Exercise 10
Create a method called `catchPokemon` and add it to the `game` object. You should not need to edit
 the original game object directly. This method should:
...
// Use the catchPokemon method to pass "Slowpoke" to the party
game.catchPokemon(slowpoke);

// Print the updated party
...
for (let gymId = 0; gymId < game.gyms.length; gymId++) {
  if (game.gyms[gymId].difficulty < 6) {
      game.gyms[gymId].completed = true
  }
}
...
game.gymStatus = function () {
  ...
  // Log the final gym tally
  console.log("Gym Status:", gymTally);
};

// Call the gymStatus method to display the tally
game.gymStatus();
...
for (let gymId = 0; gymId < game.gyms.length; gymId++) {
  if (game.gyms[gymId].difficulty < 8) {
      game.gyms[gymId].completed = true
  }
}
